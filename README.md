...
The following is a ```.so``` of [InfInt.h](https://github.com/sercantutar/infint/blob/master/InfInt.h) which was made by Sercantutar, this compiled Library has the ability of working with Infinite Integers within the IOS and Android development. Although, the library does have drawback, it does not support any cross-variable types, so can not run simultaneously with ```long long``` ```int``` or anything. Now, you can create any Mobile Application using this ```.so``` that can support infinity of number calculations without throwing an Overflow error, No Overflows in your calculation again. 
